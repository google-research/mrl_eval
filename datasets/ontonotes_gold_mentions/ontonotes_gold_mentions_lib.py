...
"""Implementation of Dataset class for OntoNotes with gold mentions."""
...
class OntoNotesGoldMentions(dataset_lib.Dataset):
  """Implementation of a Dataset class for OntoNotes with gold mentions."""
  ...
  def process_raw_examples(
    ...
    for raw_example in dataset:
      ...
      if self._drop_singleton_clusters:
        clusters_with_char_indices = [
            cluster
            for cluster in clusters_with_char_indices
            if len(cluster) > 1
        ]
